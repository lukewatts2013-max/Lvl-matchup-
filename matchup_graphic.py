...
# ── SYSTEM FONT PATHS ──────────────────────────────────────────────────────────
# Bundled fonts (shipped alongside this script) are tried FIRST — this makes
# rendering consistent across hosts. Relying only on system-installed fonts
# (e.g. /usr/share/fonts/...) breaks silently on hosts that don't have them
# preinstalled (seen when moving from Wispbyte to Railway): Pillow falls back
# to its built-in bitmap font, which ignores every requested size and renders
# everything tiny — the exact "all lettering is small" symptom.
...
def _font(size: int, bold: bool = False) -> ImageFont.ImageFont:
    bundled  = _BUNDLED_BOLD if bold else _BUNDLED_REGULAR
    primary  = _DEJAVU_BOLD    if bold else _DEJAVU_REGULAR
    fallback = _DEJAVU_REGULAR if bold else _DEJAVU_BOLD
    for path in (bundled, primary, fallback, "DejaVuSans-Bold.ttf", "DejaVuSans.ttf"):
        try:
