...
def _gap_factor(opp_ovr: int, player_ovr: int) -> float:
    """
    Difficulty factor based on the GAP between this specific player's own
    off_ovr and the opponent's OVR, rather than a one-size-fits-all
    baseline. A 230-rated player facing a 260 defense (30-point gap) gets
    a much smaller penalty than a 150-rated player facing that same 260
    (110-point gap) — the flat generic curve treats them identically.
    No bonus for a favorable gap (capped at 1.0), only a penalty for an
    unfavorable one — matches the flat curve's floor/rate for consistency.
    """
...
def predict_score(eff_ppd: float, opp_ovr: int | None = None, player: dict | None = None) -> int:
    ...
    Three-tier hybrid, in priority order:
      1. PERSONAL SLOPE (results-first) — once a player has enough logged
         games against a real variety of opponent OVRs (see
         _personal_difficulty_slope), their own actual track record
         against tough/easy competition drives the prediction. This is
         the long-run source of truth — a player's real results always
         win out over what their rating alone would predict.
      2. OVR-GAP BASELINE (cold-start) — before there's enough history to
         trust a personal trend, and only if this player's own off_ovr is
         on file, difficulty is judged by the gap between their OVR and
         the opponent's, instead of a single league-wide baseline. Fades
         out automatically the moment tier 1 has enough data to take over.
      3. FLAT GENERIC CURVE (fallback) — no history and no off_ovr on
         file: same curve everyone used before either refinement existed.
    ...
    if opp_ovr:
        ...
        if fit is not None:
            # Tier 1: enough real results — trust them over any rating.
            ...
            base += nudge
        elif player is not None and player.get("off_ovr"):
            # Tier 2: no track record yet, but we know this player's own
            # rating — use the gap between it and the opponent's OVR.
            base *= _gap_factor(opp_ovr, player["off_ovr"])
        else:
            # Tier 3: neither — same curve as always.
            factor = max(0.78, 1.0 - max(0, opp_ovr - 220) * 0.0012)
            base *= factor
