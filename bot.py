...
def _diff_factor(opp_ovr: int, player_ovr: int | None = None) -> float:
    """
    Difficulty multiplier based on the GAP between a player's own OVR and
    the opponent's — not a fixed league-wide baseline. This keeps working
    correctly no matter what OVR scale the league is on, or how much
    everyone's ratings climb over a season, since it only ever compares
    two players against each other in the moment, never against a magic
    number that goes stale as the whole league's ratings move. No bonus
    for a favorable gap (capped at 1.0), only a penalty for an
    unfavorable one. If player_ovr isn't known, returns 1.0 (neutral)
    rather than guessing.
    """
    if player_ovr is None:
        return 1.0
    gap = max(0, opp_ovr - player_ovr)
    return max(0.78, 1.0 - gap * 0.0012)


def _game_efficiency(entry: dict) -> float:
    """Difficulty-adjusted PPD for one logged game, based on the GAP
    between the player's OWN OVR at the time (snapshotted on the entry
    as "player_ovr" when it was logged) and that game's opponent OVR —
    not a fixed baseline. Falls back to a neutral (no adjustment) factor
    for older entries logged before this was tracked, or if the
    player's OVR was never on file. Divides by the game's actual real-
    drive count (default 3), so a round with 1 or 2 fumbles — fewer real
    scoring attempts — isn't unfairly diluted by dividing points across
    drives that never had a chance to score."""
    drives = entry.get("drives", 3) or 3
    factor = _diff_factor(entry.get("opp_ovr", 0), entry.get("player_ovr"))
    return (entry.get("points", 0) / drives) / factor
...
def composite_score(player: dict) -> float:
    ...
    each difficulty-adjusted per game, PLUS a trend bonus (see
    _ladder_trend_bonus) for players who've proven — through real,
    logged results — that they specifically play up against tougher
    competition. Two players with an identical raw average no longer
    rank identically if one of them has earned that extra trust.

    This naturally smooths out a single bad (or great) day — it's a
    fraction of a fraction of the total, not a third of the whole score
    — while still letting real, sustained recent form move a player up
    or down faster than career average alone would. If a player has no
    games in the 7-day or 3-day window (new to the roster, or hasn't
    played recently), that window falls back to their career efficiency
    instead of penalising them for the gap.
    """
    ...
    if three is None:
        three = career
    base  = 0.50 * career + 0.25 * seven + 0.25 * three
    trend = _ladder_trend_bonus(player)
    return base + trend
...
_MAX_LADDER_TREND_BONUS = 1.5   # cap for the same slope's effect on RANK, not just EXP
_TREND_PROJECTION_GAP   = 15    # project the slope to "opponent 15 OVR tougher than usual"
...
def _personal_difficulty_slope(player: dict):
    ...
    if len(history) < _MIN_GAMES_FOR_PERSONAL:
        return None
    ovrs = [m.get("opp_ovr", 100) for m in history]
...
def _ladder_trend_bonus(player: dict) -> float:
    """
    Turns the same personal difficulty slope used for EXP predictions into
    a bonus/penalty on the actual ladder score, so "proven to play up"
    affects RANK, not just the scoreboard's expected-score number. Projects
    the slope forward to a moderately tougher-than-average matchup
    (_TREND_PROJECTION_GAP OVR points above what they've actually faced)
    and adds that projected swing directly to the ladder score, capped so
    one noisy trend can't outweigh the whole rest of the formula. Returns
    0.0 (no effect) until there's enough real data to trust the fit.
    """
    fit = _personal_difficulty_slope(player)
    if fit is None:
        return 0.0
    slope, _mean_ovr = fit
    bonus = slope * _TREND_PROJECTION_GAP
    return max(-_MAX_LADDER_TREND_BONUS, min(_MAX_LADDER_TREND_BONUS, bonus))
...
def predict_score(eff_ppd: float, opp_ovr: int | None = None, player: dict | None = None) -> int:
    ...
    Pure personal-gap model — difficulty is always judged by the gap
    between THIS player's own OVR and the specific opponent's OVR, never
    a fixed league-wide baseline. This keeps working correctly no matter
    what OVR scale the league is on, or how much everyone's OVR climbs
    over the course of a season — there's no magic number to go stale.

    Two signals layer together:
      1. THE GAP ITSELF — (opponent_OVR − this player's own OVR), always
         applied first if this player's off_ovr is on file. If it isn't,
         no difficulty adjustment happens at all (neutral 1.0) rather
         than guessing from a baseline that may not mean anything at the
         league's current OVR scale.
      2. PERSONAL TREND (results-first, layered on top) — once a player
         has enough logged games spanning real opponent-OVR variety (see
         _personal_difficulty_slope), their own actual track record
         nudges the prediction further — a player who's proven they hold
         up better (or worse) than their raw OVR gap alone would suggest
         gets credit for that, capped so a small/noisy sample can't
         overcorrect.
    ...
    base = eff_ppd * 3
    if opp_ovr:
        player_ovr = player.get("off_ovr") if player is not None else None
        base *= _diff_factor(opp_ovr, player_ovr)

        fit = _personal_difficulty_slope(player) if player is not None else None
        if fit is not None:
            slope, mean_ovr = fit
...
DEFAULT_PLAYERS = {
    ...
    "SunDevilTyler": {"name": "SunDevilTyler", "weekly": 24, "yearly": 24, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 7.05},
    "Thrillhouse":   {"name": "Thrillhouse",   "weekly": 22, "yearly": 22, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 7.48},
    "mike9413":      {"name": "mike9413",       "weekly": 16, "yearly": 16, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 6.64},
    "DAGOAT":        {"name": "DAGOAT",         "weekly": 24, "yearly": 24, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 7.22},
    # Tier 2
    "HeroOfWild":    {"name": "HeroOfWild",     "weekly": 22, "yearly": 22, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 6.87},
    "Magicmikey66":  {"name": "Magicmikey66",   "weekly": 24, "yearly": 24, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 6.70},
    "bohica7599":    {"name": "bohica7599",      "weekly": 24, "yearly": 24, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 6.09},
    "Skoltrain":     {"name": "Skoltrain",      "weekly": 20, "yearly": 20, "total_drives": 3, "ppd_drives": 3, "is_benched": False, "career_avg_ppd": 6.40},
    # Tier 3
    "Raks":          {"name": "Raks",           "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 6.24},
    "Ixyjakobe":     {"name": "Ixyjakobe",      "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 6.13},
    "Kdaddy99":      {"name": "Kdaddy99",       "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 5.86},
    "DirtyBirds559": {"name": "DirtyBirds559",  "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 5.56},
    # Tier 4
    "Kirito":        {"name": "Kirito",         "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 5.99},
    "Swarm":         {"name": "Swarm",          "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 5.72},
    "Leroiheenok":   {"name": "Leroiheenok",    "weekly": 0,  "yearly": 0,  "total_drives": 0, "ppd_drives": 0, "is_benched": False, "career_avg_ppd": 6.38},
    }
...
def load_data():
    ...
    # ── Persistent ladder order ─────────────────────────────────────────────
    # Rank position is now sticky state, not recalculated fresh every time —
    # it only moves via the capped ±2/round logic in !endround. One-time
    # bootstrap: if there's no ladder yet, seed it from current composite_score
    # so the starting order reflects real standings rather than roster order.
    if "ladder_order" not in data:
        active_names = [
            p.get("name", pid) for pid, p in data["players"].items()
            if not p.get("is_benched", False)
        ]
        active_names.sort(key=lambda n: composite_score(find_player_by_name(data, n)[1]), reverse=True)
        data["ladder_order"] = active_names
    return data


def offense_only_weekly_ppd(player: dict) -> float:
    """
    True offense-only PPD for the current week. The raw weekly/ppd_drives
    figure mixes bonus D points (pick6/pick2/safety) into the numerator
    even though those were never tied to a drive — this instead sums the
    actual match_history points (which never include bonus plays) for
    the most recent entries whose drives add up to ppd_drives, matching
    MADDBOT's approach of keeping offense and defense scoring separate.
    """
    target_drives = player.get("ppd_drives", 0)
    if target_drives <= 0:
        return 0.0
    pts_accum, drives_accum = 0, 0
    for m in reversed(player.get("match_history", [])):
        if drives_accum >= target_drives:
            break
        pts_accum += m.get("points", 0)
        drives_accum += m.get("drives", 3)
    return pts_accum / target_drives


def ladder_rank_key(data: dict, player_name: str) -> int:
    """
    Sort key for display/matchup order: position in the persistent
    ladder_order (0 = rank 1). Falls back to appending at the bottom for
    any player missing from ladder_order (shouldn't normally happen —
    !addplayer/!removeplayer keep it in sync — but this keeps sorting
    from crashing if it ever drifts out of sync).
    """
    order = data.get("ladder_order", [])
    try:
        return order.index(player_name)
    except ValueError:
        return len(order) + 1_000_000
...
async def create_matchup(ctx, *, raw_input: str):
    ...
    try:
        ...
        for p_id, p_stats in data["players"].items():
            ...
            # If no drives yet this week, fall back to career avg PPD
            ppd  = offense_only_weekly_ppd(p_stats) if ppd_drives > 0 else float(p_stats.get("career_avg_ppd") or 0)
        ...
        # Best ladder position vs toughest opponent; cap at 16 active slots
        roster_sorted = sorted(roster_performance, key=lambda x: ladder_rank_key(data, x["name"]))[:16]
...
async def repost_matchup(ctx, *, channel=None, purge=True):
    ...
    # persistent ladder position (only moves via the capped !endround logic).
    opp_league    = matchup.get("opp_league")
    matchups_dict = {}

    def _sort_key(p):
        return ladder_rank_key(data, p.get("name", ""))

    for p_stats in sorted(data["players"].values(), key=_sort_key):
        if p_stats.get("is_benched", False):
            continue
...
async def set_match(ctx, player_name: str, opponent_name: str, opp_ovr: int):
    ...
    Usage: !setmatch SunDevilTyler SkattPack 105
    Creates the assignment if it doesn't exist yet.
    """
    if not (50 <= opp_ovr <= 200):
        await ctx.send("⚠️ OVR must be between 50 and 200.")
        return
...
async def prep(ctx):
    ...
    for p_id, p_stats in data["players"].items():
        ppd_drives = p_stats.get("ppd_drives", 0)
        weekly     = p_stats.get("weekly", 0)
        raw_ppd    = offense_only_weekly_ppd(p_stats)
        eff        = calc_efficiency(p_stats)
    ...
    active.sort( key=lambda x: ladder_rank_key(data, x["name"]))
    benched.sort(key=lambda x: ladder_rank_key(data, x["name"]))
    ...
    embed = discord.Embed(
        title=f"📋 ROSTER PREP — {total} Players",
...
async def add_player(ctx, name: str, off_ovr: int = 0):
    ...
    async with _data_lock:
        ...
        data["players"][name] = {
            ...
            "off_ovr": off_ovr,
            "match_history": [],
            "weekly_allowed": 0, "yearly_allowed": 0,
        }
        # New players start at the bottom of the ladder and earn their way
        # up under the same capped ±2/round movement as everyone else.
        data.setdefault("ladder_order", []).append(name)
        save_data(data)
    await _try_delete(ctx.message)
...
async def remove_player(ctx, *, name: str):
    ...
    async with _data_lock:
        ...
        # Also remove them from the persistent ladder
        ladder = data.get("ladder_order", [])
        if canonical in ladder:
            ladder.remove(canonical)
...
async def set_ovr(ctx, player_name: str, ovr: int):
    """Admin: set any player's offensive OVR: !setovr PlayerName 105"""
    if not (50 <= ovr <= 200):
        await ctx.send("⚠️ OVR must be between 50 and 200.")
        return
    async with _data_lock:
...
async def my_ovr(ctx, ovr: int):
    """Set your own offensive OVR (no admin required): !myovr 105"""
    if not (50 <= ovr <= 200):
        await ctx.send("⚠️ OVR must be between 50 and 200.")
        return
    async with _data_lock:
...
async def bench_player(ctx, *, player_name: str):
    ...
    async with _data_lock:
        ...
        if player is None:
            # Fall back to treating it as an @mention/ID, in case that's
            # how it was typed — resolve to their actual Discord name(s)
            # and try those against the roster too.
            stripped = raw.strip("<@!>")
            if stripped.isdigit():
    ...
    async with _data_lock:
        ...
        if real_drives > 0:
            ...
            player["match_history"].append({
                ...
                "date": datetime.now().strftime("%Y-%m-%d"),
                "drives": real_drives, "player_ovr": player.get("off_ovr"),
            })
...
async def combo(ctx, *args):
    ...
    async with _data_lock:
        ...
        player.setdefault("match_history", []).append({
            ...
            "date": datetime.now().strftime("%Y-%m-%d"), "player_ovr": player.get("off_ovr"),
        })
        ...
        player["yearly_allowed"] = player.get("yearly_allowed", 0) + def_points
...
async def fix_gave(ctx, *args):
    ...
    async with _data_lock:
        ...
        player["yearly_allowed"] = max(0, player.get("yearly_allowed", 0) + diff)
...
async def show_stats(ctx, timeframe: str = "weekly"):
    ...
    """
    tf = timeframe.lower()
    if tf not in ["weekly", "yearly", "alltime"]:
        await ctx.send("Use `!stats weekly`, `!stats yearly`, or `!stats alltime`!")
        return
...
async def clear_stats(ctx, timeframe: str = "weekly"):
    """Usage: !clearstats weekly | !clearstats yearly"""
    tf = timeframe.lower()
    if tf not in ["weekly", "yearly"]:
        await ctx.send("Use `!clearstats weekly` or `!clearstats yearly`!")
        return
...
async def gave(ctx, *args):
    ...
    async with _data_lock:
        ...
        player["yearly_allowed"] = player.get("yearly_allowed", 0) + points
...
async def gave_pick6(ctx, *args):
    ...
    async with _data_lock:
        ...
        player["yearly_allowed"] = player.get("yearly_allowed", 0) + 6
...
async def gave_pick2(ctx, *args):
    ...
    async with _data_lock:
        ...
        player["yearly_allowed"] = player.get("yearly_allowed", 0) + 2
...
async def send_live_leaderboard(ctx, update_msg=""):
    ...
    for key, stats in data["players"].items():
        ...
        raw_ppd    = offense_only_weekly_ppd(stats)
        ...
        # Yearly net +/- avg per game
        yearly_pts      = stats.get("yearly", 0)
        yearly_allowed  = stats.get("yearly_allowed", 0)
        yearly_net      = yearly_pts - yearly_allowed
        games_played    = ppd_drives // 3 if ppd_drives >= 3 else 0
        yr_avg_net      = yearly_net / games_played if games_played > 0 else 0.0
        ...
        leaderboard_data.append({
            ...
            "career": career, "net": net_weekly, "yr_avg_net": yr_avg_net,
            "expected": exp, "benched": stats.get("is_benched", False),
        })

    leaderboard_data.sort(key=lambda x: ladder_rank_key(data, x["name"]))

    lines = []
    for idx, p in enumerate(leaderboard_data, 1):
        icon       = "🟢" if p["eff"] >= 6.0 else "🔴"
        net_str    = f"+{p['net']}"    if p["net"]        >= 0 else str(p["net"])
        yr_str     = f"+{p['yr_avg_net']:.1f}" if p["yr_avg_net"] >= 0 else f"{p['yr_avg_net']:.1f}"
...
async def end_round(ctx):
    ...
    Pure stats, no win/loss record: each player's weekly totals (Total,
    PPD, Net) alongside their career numbers (EFF, All-Time PPD), with a
    ✅/❌ flag for whether they're at or above 6.0 PPD this week.
    Run !endweek when the full week is done.
    """
    print("[DEBUG] endround ENTERED", flush=True)
    # ── Delete the triggering command first ───────────────────────────────────
    await _try_delete(ctx.message)

    # ── Purge every message in the channel ───────────────────────────────────
    print("[DEBUG] endround starting purge", flush=True)
    try:
        deleted = await ctx.channel.purge(limit=1000)
        print(f"[DEBUG] endround purge complete, deleted={len(deleted)}", flush=True)
    except discord.Forbidden:
        print("[DEBUG] endround purge Forbidden, falling back to one-by-one", flush=True)
        ...
        async for msg in ctx.channel.history(limit=1000):
            try:
                ...
                pass
        print(f"[DEBUG] endround one-by-one delete complete, deleted={count}", flush=True)
    except Exception as e:
        print(f"[DEBUG] endround purge raised unexpected error: {type(e).__name__}: {e}", flush=True)
        await ctx.send(f"❌ `!endround` failed during channel purge: `{type(e).__name__}: {e}`")
        return
    ...
    try:
        ...
        def _alltime_ppd(ps):
            mh = ps.get("match_history", [])
            if not mh:
                return ps.get("career_avg_ppd")
            total_pts = sum(m.get("points", 0) for m in mh)
            total_drv = sum(m.get("drives", 3) for m in mh)
            return (total_pts / total_drv) if total_drv > 0 else ps.get("career_avg_ppd")
        ...
        for pname, asgn in sorted(assignments.items(), key=lambda kv: _weekly(kv[0]), reverse=True):
            ...
            ppd         = round(offense_only_weekly_ppd(ps), 2) if ps and ppd_drives > 0 else 0.0
            eff         = round(calc_efficiency(ps), 2) if ps else 0.0
            at_ppd      = _alltime_ppd(ps) if ps else None
            at_ppd_str  = f"{at_ppd:.2f}" if at_ppd is not None else "—"
            net         = our_sc - gave_up
            net_str     = f"+{net}" if net > 0 else str(net)
            icon        = "✅" if ppd >= 6.0 else "❌"

            lines.append(
                f"{icon} **{pname}**\n"
                f"  ↳ Weekly: Total `{our_sc}` · PPD `{ppd}` · Net `{net_str}` (gave up `{gave_up}`)\n"
                f"  ↳ Career: EFF `{eff}` · All-Time PPD `{at_ppd_str}`"
            )

        # ── Persistent ladder movement (capped ±2 spots per round) ──────────
        # Rank position is sticky state, not recalculated from scratch. A
        # player only becomes vulnerable to being passed if their MOST
        # RECENT round's offense (pure, no bonus points) was below 14 —
        # and even then, only actually swaps if the challenger directly
        # below them genuinely has a better recent (3-day) offense-only
        # trend, not just a single lucky game. Capped at 2 total swaps
        # per player per round, however sustained the slump is — repeated
        # bad rounds compound across MULTIPLE !endround calls naturally,
        # since this same cap reapplies fresh each time.
        ladder = data.setdefault("ladder_order", [])
        for pname in assignments:
            if pname not in ladder:
                ladder.append(pname)

        def _this_round_offense(ps):
            mh = ps.get("match_history", [])
            return mh[-1].get("points", 0) if mh else 0

        def _recent_form(ps):
            windowed = calc_windowed_efficiency(ps, 3)
            return windowed if windowed is not None else calc_efficiency(ps)
        ...
        while i < len(ladder):
            pname = ladder[i]
            _, ps = find_player_by_name(data, pname)
            if not ps or ps.get("is_benched", False) or pname not in assignments:
                i += 1
                continue
            if _this_round_offense(ps) < 14:
                moved = 0
                while moved < 2 and i + 1 < len(ladder):
                    challenger_name = ladder[i + 1]
                    _, challenger = find_player_by_name(data, challenger_name)
                    if not challenger or challenger.get("is_benched", False):
                        break
                    if _recent_form(challenger) > _recent_form(ps):
                        ladder[i], ladder[i + 1] = ladder[i + 1], ladder[i]
                        movement_log.append(f"{challenger_name} passed {pname}")
                        i += 1
                        moved += 1
                    else:
                        break
            i += 1
        if movement_log:
            print(f"[DEBUG] endround ladder movement: {movement_log}", flush=True)

        try:
            save_data(data)
        except RuntimeError as e:
            print(f"[ERROR] endround could not save ladder movement: {e}", flush=True)
            await ctx.send(f"⚠️ Ladder movement computed but could not be saved: `{e}`")

        embed = discord.Embed(
            title="🔔 Round Complete",
            description="Scores carry into next round — run `!endweek` when the full week is done.",
        ...
        print("[DEBUG] endround sending embed", flush=True)
        await ctx.channel.send(embed=embed)
        print("[DEBUG] endround embed sent successfully", flush=True)
    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        print(f"[ERROR] endround raised:\n{tb}", flush=True)
        await ctx.send(f"❌ `!endround` failed while building the summary: `{type(e).__name__}: {e}`")
...
async def test_all(ctx):
    ...
    try:
        ...
        try:
            ...
            predict_score(6.5, 110)
            record("core scoring helpers", True)
        except Exception as e:
            record("core scoring helpers", False, f"{type(e).__name__}: {e}")

        # ── Ladder trend bonus: rewards playing up, not just raw average ────
        try:
            plays_up = {"match_history": [
                {"points": 15, "opp_ovr": 85,  "player_ovr": 90, "drives": 3},
                {"points": 16, "opp_ovr": 90,  "player_ovr": 90, "drives": 3},
                {"points": 22, "opp_ovr": 110, "player_ovr": 90, "drives": 3},
                {"points": 24, "opp_ovr": 115, "player_ovr": 90, "drives": 3},
            ]}
            fades = {"match_history": [
                {"points": 24, "opp_ovr": 85,  "player_ovr": 90, "drives": 3},
                {"points": 22, "opp_ovr": 90,  "player_ovr": 90, "drives": 3},
                {"points": 16, "opp_ovr": 110, "player_ovr": 90, "drives": 3},
                {"points": 15, "opp_ovr": 115, "player_ovr": 90, "drives": 3},
            ]}
            bonus_up    = _ladder_trend_bonus(plays_up)
            bonus_fade  = _ladder_trend_bonus(fades)
            new_player  = {"match_history": [{"points": 20, "opp_ovr": 100, "player_ovr": 90, "drives": 3}]}
            bonus_new   = _ladder_trend_bonus(new_player)
            ok = (bonus_up > 0) and (bonus_fade < 0) and (bonus_up > bonus_fade) and (bonus_new == 0.0)
            detail = f"plays_up={bonus_up:+.2f}, fades={bonus_fade:+.2f}, dormant(1 game)={bonus_new:+.2f}"
            record("ladder trend bonus rewards playing up / penalizes fading", ok, detail)
        except Exception as e:
            record("ladder trend bonus rewards playing up / penalizes fading", False, f"{type(e).__name__}: {e}")
        ...
        TEST_CALLS = {
            ...
            "removeplayer": (("__Test_Dummy__",), {}),
            "setovr":      ((roster_name, 220), {}),
            "myovr":       ((220,), {}),
            "bench":       ((ctx.author.display_name,), {}),
            "active":      ((ctx.author.display_name,), {}),
        ...
        order = [
            ...
            "addplayer", "removeplayer", "setovr", "myovr", "bench", "active", "register",
            "postmatchup",
        ...
        # ── !score ... fumble2 (partial fumble: only 1 of 3 drives real) ────
        try:
            _, before = find_player_by_name(sandbox, p2)
            before_ppd_drives  = before.get("ppd_drives", 0)
            before_total_drives = before.get("total_drives", 0)
            before_history_len = len(before.get("match_history", []))

            local_ctx = _FakeCtx(ctx)
            await score_cmd.callback(local_ctx, p2, "16", "fumble2")

            _, after = find_player_by_name(sandbox, p2)
            after_ppd_drives   = after.get("ppd_drives", 0)
            after_total_drives = after.get("total_drives", 0)
            after_history_len  = len(after.get("match_history", []))
            new_entry = after.get("match_history", [])[-1] if after_history_len > before_history_len else None

            ok = (
                after_total_drives == before_total_drives + 3   # all 3 drives were played
                and after_ppd_drives == before_ppd_drives + 1   # only 1 real (non-fumbled) drive
                and after_history_len == before_history_len + 1
                and new_entry is not None
                and new_entry.get("drives") == 1
                and new_entry.get("points") == 16
            )
            detail = (
                f"total_drives +{after_total_drives - before_total_drives} (expected +3), "
                f"ppd_drives +{after_ppd_drives - before_ppd_drives} (expected +1), "
                f"new match_history entry drives={new_entry.get('drives') if new_entry else 'MISSING'}"
            )
            record("!score ... fumble2 credits only the real drive(s)", ok, detail)
        except Exception as e:
            record("!score ... fumble2 credits only the real drive(s)", False, f"{type(e).__name__}: {e}")
        ...
        # summary must still reflect each player's real weekly/career
        # totals (pulled from their own persistent player record) — not
        # get corrupted by the live `assignments` dict being fully
        # overwritten the moment !matchup runs again for the new pairing.
        try:
            _, p1_before = find_player_by_name(sandbox, p1)
            expected_weekly = p1_before.get("weekly", 0)
            expected_ppd_drives = p1_before.get("ppd_drives", 0)
            expected_ppd = round(expected_weekly / expected_ppd_drives, 2) if expected_ppd_drives > 0 else 0.0
            ...
            ok = (
                bool(p1_block)
                and f"Total `{expected_weekly}`" in p1_block
                and f"PPD `{expected_ppd}`" in p1_block
                and "ZZRegOpp" not in p1_block
            )
            detail = f"expected weekly={expected_weekly}, ppd={expected_ppd} found in p1's block: {ok}"
            record("!endround stats survive an overwritten future pairing", ok, detail)
        except Exception as e:
            record("!endround stats survive an overwritten future pairing", False, f"{type(e).__name__}: {e}")

        # ── !removeplayer actually deletes and cleans up matchup assignment ──
        try:
            remove_cmd  = bot.get_command("removeplayer")
            addplayer_cmd = bot.get_command("addplayer")

            before_count = len(sandbox.get("players", {}))
            local_ctx = _FakeCtx(ctx)
            await addplayer_cmd.callback(local_ctx, "__RemovalTarget__", 200)
            # Assign them into the current matchup to verify cleanup too
            sandbox.setdefault("current_matchup", {}).setdefault("assignments", {})["__RemovalTarget__"] = {
                "opponent_name": "ZZDeleteMeOpp", "opponent_ovr": 220
            }
            after_add = len(sandbox.get("players", {}))

            local_ctx2 = _FakeCtx(ctx)
            await remove_cmd.callback(local_ctx2, name="__RemovalTarget__")
            after_remove = len(sandbox.get("players", {}))
            _, still_there = find_player_by_name(sandbox, "__RemovalTarget__")
            still_assigned = "__RemovalTarget__" in sandbox.get("current_matchup", {}).get("assignments", {})

            ok = (
                after_add == before_count + 1
                and after_remove == before_count
                and still_there is None
                and not still_assigned
            )
            detail = f"count {before_count}->{after_add}->{after_remove}, still_in_roster={still_there is not None}, still_assigned={still_assigned}"
            record("!removeplayer deletes player and clears matchup assignment", ok, detail)
        except Exception as e:
            record("!removeplayer deletes player and clears matchup assignment", False, f"{type(e).__name__}: {e}")

        # ── Persistent ladder movement: capped swap actually happens ────────
        try:
            ladder = sandbox.setdefault("ladder_order", [])
            for pname in (p1, p2):
                if pname not in ladder:
                    ladder.append(pname)
            # Force p1 directly above p2 in the ladder, give p1 a bad recent
            # round (<14) and p2 a clearly better one, both dated today.
            ladder.remove(p1); ladder.remove(p2)
            ladder.append(p1); ladder.append(p2)
            idx1, idx2 = ladder.index(p1), ladder.index(p2)

            today_str = datetime.now().strftime("%Y-%m-%d")
            _, p1_stats = find_player_by_name(sandbox, p1)
            _, p2_stats = find_player_by_name(sandbox, p2)
            p1_stats.setdefault("match_history", []).append(
                {"points": 8, "opp_ovr": 220, "opponent": "ZZLow", "date": today_str, "drives": 3})
            p2_stats.setdefault("match_history", []).append(
                {"points": 24, "opp_ovr": 220, "opponent": "ZZHigh", "date": today_str, "drives": 3})
            p1_stats["ppd_drives"] = p1_stats.get("ppd_drives", 0) + 3
            p2_stats["ppd_drives"] = p2_stats.get("ppd_drives", 0) + 3

            endround_cmd = bot.get_command("endround")
            local_ctx = _FakeCtx(ctx)
            await endround_cmd.callback(local_ctx)

            new_ladder = sandbox.get("ladder_order", [])
            swapped = (idx1 == idx2 - 1) and (new_ladder.index(p2) == idx1 and new_ladder.index(p1) == idx2)
            detail = f"before: {p1}@{idx1},{p2}@{idx2} | after: {p1}@{new_ladder.index(p1)},{p2}@{new_ladder.index(p2)}"
            record("!endround ladder movement swaps a struggling player down", swapped, detail)
        except Exception as e:
            record("!endround ladder movement swaps a struggling player down", False, f"{type(e).__name__}: {e}")
        ...
        # once active once benched" bug this guards against. Also confirms
        # plain roster names work directly, with no @mention required.
        try:
            ...
            local_ctx = _FakeCtx(ctx)
            await bench_cmd.callback(local_ctx, player_name=p1)
            local_ctx2 = _FakeCtx(ctx)
            await active_cmd.callback(local_ctx2, player_name=p1)
...
# Accept either variable name — different hosts/past setups have used either.
token = os.environ.get("DISCORD_BOT_TOKEN") or os.environ.get("DISCORD_TOKEN")
if not token:
    raise RuntimeError(
        "No bot token found. Set an environment variable named either "
        "DISCORD_BOT_TOKEN or DISCORD_TOKEN with your bot's token."
    )
